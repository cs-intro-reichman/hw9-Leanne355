...
public class MemorySpace {
	...
	public void free(int address) {
		...
		while (iterator.hasNext()) {
			MemoryBlock aloccatedBlock = iterator.next();
			if (aloccatedBlock.baseAddress == address) {
				allocatedList.remove(aloccatedBlock);
				freeList.addLast(aloccatedBlock);
			}
	...
	public void defrag() {
		...
		while (iterator1.hasNext()) {
			MemoryBlock defragBlock1 = iterator1.next();
			ListIterator iterator2 = freeList.iterator();
			while (iterator2.hasNext()) {
				MemoryBlock defragBlock2 = iterator2.next();
				if (defragBlock1.baseAddress + defragBlock1.length == defragBlock2.baseAddress){
					defragBlock1.length += defragBlock2.length;
					freeList.remove(defragBlock2);
